...
export default {
  ...
  mounted() {
    ...
    this.echartsUserAction = _this.$echarts.init(document.getElementById('echarts-moth-user'), 'macarons')
    // eslint-disable-next-line no-undef
    this.echartsQuestion = _this.$echarts.init(document.getElementById('echarts-moth-question'), 'macarons')
    this.loading = true
    getDashboardCharts().then(res => {
      let data = res.data
      _this.examPaperCount = data.examPaperCount
      _this.questionCount = data.questionCount
      _this.doExamPaperCount = data.doExamPaperCount
      _this.doQuestionCount = data.doQuestionCount
      _this.echartsUserAction.setOption(this.option('用户活跃度', '{b}日{c}度', data.monthDayText, data.monthDayUserActionValue))
      _this.echartsQuestion.setOption(this.option('题目月数量', '{b}日{c}题', data.monthDayText, data.monthDayDoExamQuestionValue))
      this.loading = false
    })
  },
  methods: {
